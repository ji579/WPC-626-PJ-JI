...
function loadExhibitionData(exhibitIndex = 0) { 
    if (!exhibInfo || exhibInfo.length === 0 || !exhibInfo[exhibitIndex]) {
        console.error(`Exhibition data for index ${exhibitIndex} is missing or empty.`);
        return;
    }
    ...
    if(imgtxt1El) imgtxt1El.innerHTML = data.imgtxt1.replace(/\n/g, '<br>').trim();
    ...
    if(imgtxt3El) imgtxt3El.innerHTML = data.imgtxt3.replace(/\n/g, '<br>').trim();
    ...
    if (exbtxtElement) {
        const paragraphs = data.exbtxt.split('\n\n').filter(p => p.trim() !== '');
        exbtxtElement.innerHTML = paragraphs.map(p => `<p>${p.trim()}</p>`).join('');
    }
    ...
    // --- 2. 이미지 로드 및 예외 처리 로직 ---
    const imgElements = [
        document.querySelector('.sa1 .modal-img img'),          
        document.querySelector('.sa3 .imgbx1 img:nth-child(1)'), 
        document.querySelector('.sa3 .imgbx1 img:nth-child(2)'), 
        document.querySelector('.sa4 .imgbx2 img:nth-child(1)'), 
        document.querySelector('.sa4 .imgbx2 img:nth-child(2)'), 
        document.querySelector('.sa4 .imgbx3 img')              
    ];
    ...
    imgElements.forEach((img, i) => {
        ...
        const handleError = function() {
            ...
            if (parentDiv) {
                ...
                if (parentDiv.classList.contains('imgbx3')) {
                     if (imgtxt4Element) imgtxt4Element.style.display = 'none';
                }
            }
...
// 
    ...
    const mainSwiperWrapper = document.querySelector(".wrap .swiper .swiper-wrapper");

    if (mainSwiperWrapper) {
        mainSwiperWrapper.innerHTML = hCode;
    } else {
        console.error("Error: Past Exhibition 목록의 .swiper-wrapper 요소를 찾을 수 없습니다.");
        return;
    }
